...
def get_real_ips():
    ...
    try:
        ...
        # 精准正则：匹配 141.101.x.x, 198.41.x.x, 104.x.x.x 等真实段位
        pattern = r'(?:141\.101|104\.|172\.|198\.41)\.\d{1,3}\.\d{1,3}'
...
def main():
    ...
    if ip_list:
        ...
        # 如果抓不到真实 IP，回退到之前“虽然-1但能上网”的域名模式
        print("未抓到有效 IP，切换回域名保底模式")
        vless_links.append(f"vless://{USER_ID}@{HOST}:{PORT}?encryption=none&security=tls&sni={HOST}&type=ws&host={HOST}&path={PATH}#域名保底_若-1请直接连接")
